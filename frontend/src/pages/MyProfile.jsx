    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      {
        ...
        ? <label htmlFor="image">
          <div className='inline-block relative cursor-pointer'>
            <img className='w-36 h-36 rounded-lg opacity-75' src={image ? URL.createObjectURL(image) : userData.image} alt="" />
            <img className='w-10 absolute bottom-12 right-12' src={image ? '' : assets.upload_icon} alt="" />
          </div>
          <input onChange={(e)=>setImage(e.target.files[0])} type="file" id='image' hidden />
        </label> 
        : <img className='w-36 h-36 rounded-lg' src={userData.image} alt="User Profile Image" />
      }
      ...
      <div className='mt-10'>
        {
          isEditing 
          ? <button className='border border-hover px-8 py-2 rounded-full hover:bg-primary hover:text-white hover:border-none transition-all' onClick={updateUserProfileData}>Save information</button>
